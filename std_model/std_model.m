...
fig_mat_hd = zeros(fig_num*fig_size(1)*fig_size(2),1);
for fig_cnt=1:1:fig_num
    fig_mat_hd(((fig_cnt-1)*fig_size(1)*fig_size(2)+1):(fig_cnt*fig_size(1)*fig_size(2)),1) = flipud(reshape(fig_mat(:,:,fig_cnt)',[fig_size(1)*fig_size(2),1]));
end
fig_mat_hd = dec2hex(fig_mat_hd,2);
writematrix(fig_mat_hd,'figure.txt');
disp('Done!');
...
writematrix(bias_mat_hd,'bias.txt');
disp('Done!');

if check_flag==1
    fid = fopen('result.txt');
    raw_result = textscan(fid,'%s');
    hd_mat = zeros(4*fig_num*weight_num,20);
    temp_char = char(zeros(40,7));
    for hd_cnt=1:2:(4*fig_num*weight_num)
        ...
        hd_result = zeros(fig_size(1)-weight_size(1)+1,fig_size(2)-weight_size(2)+1,fig_num,weight_num);
        for fig_cnt=1:1:fig_num
            for weight_cnt=1:1:weight_num
    ...
    if sum((abs(result_mat-hd_result)>=1)&(abs(abs(result_mat-hd_result)-256)>=1),'all')==0
        disp('Check pass!');
    else
        error('Check fail!');
    end
